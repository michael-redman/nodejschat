...
var db, wss, n_sockets=0, sockets=[], users=[];
...
function broadcast(time,user,message){
...
function db_notify_handler(msg){
	...
	db.query(
		'select * from chat_log where message_id=$1::integer',
		[msg.payload],
		function(err,result){
			if	(err)
				{	console.error(new Date().toISOString() + ' ' + err);
				...
				authenticate(db,session_key,ws,
					(function(ws){ return function(){
						...
						ws.on('message',(function(user){ return function(message){
							...
							db.query('select new_message(now(),$1::text,$2::text)',[users[sockets.indexOf(ws)],json.message],function(err,result){
								if	(err)
									{	console.error(new Date().toISOString()+' '+err);
										ws.close(); } });
